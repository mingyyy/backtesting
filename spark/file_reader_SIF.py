...
def load_files(from_bucket, to_bucket, app_name):
    spark = SparkSession.builder \
        .appName(app_name) \
        .config('spark.sql.files.maxPartitionBytes', 1024 * 1024 * 500) \
        .getOrCreate()
    ...
    df = spark.read.csv("s3a://" + from_bucket + "/SIF*.csv", header=True, schema=schema)
    ...
    df1.write.parquet("s3a://" + to_bucket + "/simulate_SIF1.parquet", mode="overwrite")
    df1.unpersist()

    df2=df.filter(df.year.between(1930, 1950))
    df2.write.parquet("s3a://" + to_bucket + "/simulate_SIF2.parquet", mode="overwrite")
    df2.unpersist()

    df3=df.filter(df.year.between(1950, 1970))
    df3.write.parquet("s3a://" + to_bucket + "/simulate_SIF3.parquet", mode="overwrite")
    df3.unpersist()

    df4=df.filter(df.year.between(1970, 1990))
    df4.write.parquet("s3a://" + to_bucket + "/simulate_SIF4.parquet", mode="overwrite")
    df4.unpersist()

    df5=df.filter(df.year.between(1990, 2019))
    df5.write.parquet("s3a://" + to_bucket + "/simulate_SIF5.parquet", mode="overwrite")
    df5.unpersist()
...
if __name__ == '__main__':
    load_files(bucket_larger, bucket_parquet,'load SIF_* by batch')