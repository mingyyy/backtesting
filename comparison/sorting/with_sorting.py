...
def strategy_1_all():
    ...
    new_df = df.select(df.ticker,F.max(df.date).over(w).alias('max_date')).dropDuplicates().sort(F.desc('date'))
    ...
    comparison(importing Parquet file 3.1GB):
        1. Running on t2.large: with sorting with broadcoast: #.sort(F.desc('date')) -> 2.8min
        2. Running on t2.large: without sorting with broadcoast: #.sort(F.desc('date')) -> 1.7min
