...
def send2S3(file_name, name_list):
    # send to S3
    ...
    files = glob.glob('/home/ubuntu/insight/backtesting/output/*')
    # n = 0
    for f in files:
       os.remove(f)
...
if __name__ == '__main__':
    ...
    for n in range(0, t):
        ...
        combined_csv = pd.concat([pd.read_csv("/home/ubuntu/insight/backtesting/output/" + f) for f in name_list])
        combined_csv.to_csv(r'/home/ubuntu/insight/backtesting/output/combine_csv_'+ str(n)+'.csv', index=False)
        send2S3('combine_csv_'+ str(n)+'.csv', name_list)
        # print(name_list)
